...
function ReGui:LoadPrefabs(): Folder?
	...
	local function NormalizePrefabRoot(Loaded)
		...
		local Prefabs = Loaded:FindFirstChild("Prefabs", true)
		if Prefabs then
			return Prefabs.Parent == Loaded and Loaded or Prefabs.Parent
		end
	...
	if ScriptUi then
		local Prefabs = NormalizePrefabRoot(ScriptUi)
		if Prefabs then return Prefabs end
	end
	...
	if PlayerGui then
		...
		if PlayerUI then
			local Prefabs = NormalizePrefabRoot(PlayerUI)
			if Prefabs then return Prefabs end
		end
	end

	local function TryLoad(Loader)
		local Success, Loaded = pcall(Loader)
		if not Success then return nil end
		return NormalizePrefabRoot(Loaded)
	end

	local LocalAsset = TryLoad(function()
	...
	local InsertAsset = TryLoad(function()
